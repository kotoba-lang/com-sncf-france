(ns sncf_france.main
  "Kotodama WASM entrypoint for the Sncf_france clean-room actor (L4) — Clojure port.
  ...
  facts (`sncf_france.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sncf_france")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sncf_france.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sncf_france-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sncf_france-compat/ready)
